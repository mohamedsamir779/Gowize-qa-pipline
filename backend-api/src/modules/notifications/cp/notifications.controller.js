...
class NotificationsController {
  async fetchNotifications(req, res, next) {
    try {
      const rec = await service.fetchNotifications(req.user._id, 'customers', req.query);
  ...
  async markNotificationsRead(req, res, next) {
    try {
      const rec = await service.markNotificationsRead(req.user._id, 'customers', req.body);
