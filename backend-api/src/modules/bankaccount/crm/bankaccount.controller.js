...
class BankAccountController {
  ...
  async createRecord(req, res, next) {
    try {
      ...
      const rec = await service.addBankAccount({ ...params, createdBy: req.user._id });
  ...
  async updateRecordById(req, res, next) {
    try {
      const { id } = req.params;
      const rec = await service.editBankAccount(id, { ...req.body, updatedBy: req.user._id });
  ...
  async deleteRecordById(req, res, next) {
    try {
      const { id } = req.params;
      const rec = await service.deleteBankAccount(id, { deletedBy: req.user._id });
