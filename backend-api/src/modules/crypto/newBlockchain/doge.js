...
const heartbeat = (ws, delay) => {
  logger.info(`DOGEChain: opnened connection heartbeat ==> ${delay}`);
...
class DOGEChain {
  constructor(options = {}) {
    ...
    this.websocketUrl = options.websocketUrl || `wss://socket.blockcypher.com/v1/doge/${this.testnet ? 'test3' : 'main'}`;
  ...
  GenerateKeyPairs(listen = true) {
    const ck = CoinKey.createRandom(ci('DOGE'));
